#!/usr/bin/env python3
"""
Sample Assessment Generator - WITH SIMPLIFIED RECALL SCORING

Generates 3 sample assessments using the new simplified recall approach:
1. Grade 2 ORF Assessment (Early Band)
2. Grade 2 Comprehension - Narrative (with simplified recall)
3. Grade 5 Comprehension - Nonfiction (with simplified recall)

Changes from original:
- Uses SimplifiedRecallScoringGenerator instead of RecallScoringGenerator
- Character + detail scoring approach (2 pts for both, 1 pt for either, 0 for neither)
- Much more reliable - simpler JSON structure
- Matches real-world teacher scoring practices
...
from src.generators import (
    ...
    create_simplified_recall_scoring_generator  # NEW simplified recall generator
)
...
def get_api_key_and_provider():
    ...
    provider = os.getenv('AI_PROVIDER', 'openai' if os.getenv('OPENAI_API_KEY') else 'anthropic')
...
def generate_orf_sample(api_key, provider):
    """Generate Sample 1: Grade 2 ORF Assessment"""
...
def generate_comprehension_sample_narrative(api_key, provider):
    """Generate Sample 2: Grade 2 Comprehension - Narrative (with SIMPLIFIED recall)"""
    print_header("SAMPLE 2: GRADE 2 COMPREHENSION - NARRATIVE (SIMPLIFIED RECALL)")
    ...
    print_success(f"Recall scoring: {recall.total_sentences} sentences, {recall.max_total_points} max points")
...
def generate_comprehension_sample_nonfiction(api_key, provider):
    """Generate Sample 3: Grade 5 Comprehension - Nonfiction (with SIMPLIFIED recall)"""
    print_header("SAMPLE 3: GRADE 5 COMPREHENSION - NONFICTION (SIMPLIFIED RECALL)")
...
def main():
    ...
    print("WITH SIMPLIFIED RECALL SCORING")
    print("=" * 80)
    print(f"\nTimestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Get API credentials
    api_key, provider = get_api_key_and_provider()
    print(f"✓ API key loaded ({provider})")
    
    try:
        # Generate all samples
        sample1 = generate_orf_sample(api_key, provider)
        sample2 = generate_comprehension_sample_narrative(api_key, provider)
        sample3 = generate_comprehension_sample_nonfiction(api_key, provider)
        
        # Success report
        print_header("✅ ALL SAMPLES GENERATED SUCCESSFULLY!")
        print(f"Location: {PROJECT_ROOT}/samples/")
        print("\nGenerated 6 files:")
        print("  • 3 complete assessment packages (JSON)")
        print("  • 3 manifest files")
        print("\nImprovements in this version:")
        print("  ✓ Simplified recall scoring (character + detail)")
        print("  ✓ More reliable AI responses")
        print("  ✓ Practical, assessor-friendly rubrics")
        print("  ✓ 6x less JSON complexity")
