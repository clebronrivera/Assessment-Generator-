...
class QuestionDistribution:
    ...
    def to_dict(self) -> dict:
        ...
        if self.main_idea > 0:
            result["main_idea"] = self.main_idea
        if self.text_structure > 0:
            result["text_structure"] = self.text_structure
        if self.inference_advanced > 0:
            result["inference_advanced"] = self.inference_advanced
        return result
...
class ComprehensionBlueprint:
    ...
    def __post_init__(self):
        """Validate that distribution sums to total questions."""
